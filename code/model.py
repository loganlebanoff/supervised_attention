            ...
            for idx in range(max_sent_len):
                ...
                if num_lstm == 1:
                    with tf.variable_scope("lstm"):
                        output, state = lstm(tf.concat(axis=1, values=[weighted_context, word_emb]), state)
                else:
                    with tf.variable_scope("lstm1"):
                        output1, state1 = lstm(weighted_context, state1)
    
                    with tf.variable_scope("lstm2"):
                        output, state2 = lstm(tf.concat(axis=1, values=[word_emb, output1]), state2)
                
                # Compute the logits
                expanded_output = tf.concat(axis=1, values=[output, weighted_context, word_emb])
